...
namespace Novalnet\Payment\Core;
...
class NovalnetUtil
{
    ...
    public static function getBasketDetails($oBasket, $shippingcost, $dAmount = false)
    {
        ...
        $oPaymentCost = NovalnetUtil::getTableValues('OXADDSUM', 'oxpayments', 'OXID', 'novalnetpayment');
    ...
    public static function convertOldTxnDetailsToNewFormat($iOrderNo, $aTransDetails = [])
    {
        ...
        $aAdditionalData = unserialize($aTxnTable['ADDITIONAL_DATA']);
        if(empty($aAdditionalData)) {
            $aAdditionalData = json_decode($aTxnTable['ADDITIONAL_DATA'], true);
        }
        if (!isset($aAdditionalData['novalnet_comments']) && empty($aAdditionalData['novalnet_comments'])) {
