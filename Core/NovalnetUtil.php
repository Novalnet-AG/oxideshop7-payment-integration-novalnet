...
class NovalnetUtil
{
    ...
    public static function doPayment($oOrder)
    {
        ...
        file_put_contents('payment_req.txt', print_r($aRequest, true), FILE_APPEND);
        $aPaymentResponse = self::doCurlRequest($aRequest, $sEndPoint);
        file_put_contents('payment_res.txt', print_r($aPaymentResponse, true), FILE_APPEND);
        ...
        if (!empty($aData['instalment'])) {
            ...
            if (!empty($aInstalment['cycles_executed'])) {
                ...
                for ($iCycle = 1; $iCycle <= $aInstalmentDetails['instalment_total_cycles']; $iCycle++) {
                    ...
                    if(!empty($aInstalment['cycle_dates'][ $iCycle])) {
                        $aInstalmentDetails['instalment' . $iCycle ]['next_instalment_date'] = date('Y-m-d', strtotime($aInstalment['cycle_dates'][$iCycle]));
                    }
