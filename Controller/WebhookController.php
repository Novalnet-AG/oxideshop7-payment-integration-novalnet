...
class WebhookController extends FrontendController
{
    ...
    public function getFormattedInvoiceComments()
    {
        ...
        } elseif (!empty($this->aAdditionalData['bank_details']) && !empty($aResponse['transaction']['due_date'])) {
    ...
    public function handleInstalmentCancel()
    {
        if ($this->eventData['transaction']['status'] == 'CONFIRMED') {
            ...
            if ($this->eventData['instalment']['cancel_type'] == 'ALL_CYCLES') {
                ...
                NovalnetUtil::updateTableValues('novalnet_transaction_detail', ['ADDITIONAL_DATA' => json_encode($this->aAdditionalData), 'GATEWAY_STATUS' => $this->eventData['transaction']['status'], 'CREDITED_AMOUNT' => 0, 'AMOUNT' => 0], 'ORDER_NO', $this->orderID);
    ...
    protected function getOrderReference()
    {
        ...
        if (empty($aResult['OXPAYMENTTYPE']) || $aResult['OXPAYMENTTYPE'] !== 'novalnetpayments') {
