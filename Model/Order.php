...
class Order extends Order_parent
{
 ...
 if ((isset($aResponse['payment_details']['process_mode']) && $aResponse['payment_details']['process_mode'] == 'direct') || empty(NovalnetUtil::getRequestParameter('tid'))) {
        ...
        if (!$blRecalculatingOrder) {
            ...
            $sPaymentName = !empty($this->oNovalnetSession->getVariable('sNovalnetPaymentName')) ? $this->oNovalnetSession->getVariable('sNovalnetPaymentName') : '';
            $order_id = !empty($this->oNovalnetSession->getVariable('dNnOrderNo')) ? $this->oNovalnetSession->getVariable('dNnOrderNo') : $this->oxorder__oxordernr->value;
            $this->oDb->execute('INSERT INTO novalnet_transaction_detail (ORDER_NO, PAYMENT_TYPE) VALUES (?, ?)', [$order_id, $sPaymentName]);
            }
            $this->aNovalnetData = $this->oNovalnetSession->getVariable('aNovalnetGatewayResponse');
            if (strpos($this->aNovalnetData['transaction']['payment_type'], 'INVOICE') !== false && $this->aNovalnetData['transaction']['payment_type'] === 'PREPAYMENT') {
                $aPaymentResponse = $this->updateOrderNumber();
                $this->oNovalnetSession->setVariable('aNovalnetGatewayResponse', $aPaymentResponse);
            }
            $this->logNovalnetTransaction();
            NovalnetUtil::clearNovalnetSession();
        }
        $aNovalnetResponse = $this->oNovalnetSession->getVariable('aNovalnetGatewayResponse');
        if (!isset($aNovalnetResponse['transaction']['order_no'])) {
    ...
    protected function logNovalnetTransaction()
    {
        ...
        if (in_array($this->aNovalnetData['transaction']['payment_type'], ['INVOICE', 'PREPAYMENT', 'GUARANTEED_INVOICE', 'INSTALMENT_INVOICE']) && $this->aNovalnetData['transaction']['status_code'] != '75') {
            $aInvoiceDetails = NovalnetUtil::getInvoiceComments($this->aNovalnetData);
            ...
            if (!empty($this->aNovalnetData['transaction']['partner_payment_reference'])) {
        ...
        if (!empty($this->aNovalnetData['transaction'])) {
    ...
    {
        $orderNo = $this->oxorder__oxordernr?->value ?? null;
        ...
        return NovalnetUtil::doCurlRequest($aTransactionDetails, 'transaction/update');
    }
    ...
    {
        parent::load($oOxid);
        if (($this->oxorder__oxpaymenttype?->value ?? null) === 'novalnetpayments') {
