...
jQuery(document).ready(
    function () {
        ...
        if (jQuery('form#moduleConfiguration').length) {
            ...
            jQuery('input[name="configureWebhook"]').click(function () {
                if (jQuery.trim(jQuery("#sWebhooksUrl").val()) != '') {
                    setWebhookConfig();
                }
            });
        }
                ...
                success: function (resultData) {
                    if (resultData.details == 'true') {
                        ...
                        if (response.merchant.vendor != undefined && response.merchant.project != undefined) {
                            ...
                            var sBaseUrl = window.location.origin ? window.location.origin : window.location.protocol + '//' + window.location.hostname;
                            sBaseUrl += str.replace('admin/', '');
                            var sWebhooksUrl = sBaseUrl + "?cl=WebhookController&fnc=handlerequest";
